  ...
  if (validUrl.isUri(longUrl)) {
    try {
      const url = await pool.query(
        "SELECT * FROM shortner WHERE long_url = $1",
        [longUrl]
      );
      ...
      if (url.rowCount != 0) {
        ...
        let shortUrlQuery = await pool.query(
          "INSERT INTO shortner(long_url,short_url,urlcode) VALUES($1,$2,$3) returning *",
...
router.post("/modify", async (req, res) => {
  try {
    ...
    const urlQuery = await pool.query(
      "SELECT * FROM shortner WHERE short_url=$1",
      [shortUrl]
    );
    ...
    const tempQuery = await pool.query(
      `UPDATE shortner SET expiry = expiry + INTERVAL '${days} days' WHERE short_url=$1 returning *`,
      [shortUrl]
    );
